...
// farm strategy
contract StrategyV2PairPro is StrategyV2Data, OwnableUpgradeable, IStrategyV2Pair, ICompActionTrigger {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    function initialize(address bank_, address _swapPoolImpl, address _helperImpl) public initializer {
        require(_bank == address(0), 'bank once');
        ...
        IStrategyV2SwapPoolPro(_swapPoolImpl).setStrategy(address(this));
    ...
    function emergencyWithdraw(uint256 _pid, address _account, uint256 _minOutput0, uint256 _minOutput1)
        external override onlyBank {
        // If there is a fatal error, it will be resolved by upgrading the contract
        _pid;
    }
