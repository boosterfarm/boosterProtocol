...
import "./SafeBoxCan2CTokenImplETH.sol";

contract SafeBoxCan2CTokenETH is SafeBoxCan2CTokenImplETH, ReentrancyGuard, Ownable, ICompActionTrigger, ISafeBox {
    ...
    constructor (
        ...
        address _cToken,
        address _devAddr
    ) public SafeBoxCan2CTokenImplETH(_cToken) {
        token = baseToken();
    ...
    function _repay(uint256 _bid, uint256 _value) internal {
        ...
        if(platformAmount > 0 && devAddr != address(0)) {
            IERC20(token).safeTransfer(devAddr, platformAmount);
        }
